...
`bayan-slm-engine` leverages a **Metadata-First Hybrid Grounded Distillation Pipeline** rooted in the authentic **SADA22** dataset — `MohamedRashad/SADA22`, the Saudi Audio Dataset (SADA, curated by SDAIA's National Center for AI + Saudi Broadcasting Authority; ~667 hours of Saudi speech; `cc-by-nc-sa-4.0`) — engineered to bypass massive 50GB local bandwidth bloat while enforcing strict multi-modal text routing. The bounded corpus size makes continued pretraining of the `tiiuae/Falcon-H1-0.5B-Base` foundation the pragmatic route to domain adaptation: the model inherits general Arabic/English knowledge while SFT and continued pretraining specialize it toward Saudi dialects and JSON routing.
...
flowchart TD
    ...
    C --> D["4. 16-BIT ZERO-COPY BINARY PACKING (np.memmap)<br/>~15M-30M SADA22 tokens + synthetic pairs packed into a contiguous uint16 file.<br/>Bypasses DataLoader memory bloat."]
...
* **Corpus Volume & Token Math:** The authentic baseline corpus is capped at **~15M–30M SADA22 tokens**, supplemented by controlled synthetic multi-turn JSON pairs distilled via a Frontier API. Because our custom Arabic vocabulary size is 16,000, every token ID safely fits inside an unsigned 16-bit integer (`uint16`, max value 65,535).
* **Dynamic SADA22 Field Routing:** The pipeline avoids blind text ingestion by routing metadata columns based on modality:
...
* **Verified SADA22 Schema (`MohamedRashad/SADA22`, HF datasets-server 2026-08-15):** columns `audio`, `text` (diacritics/punct preserved, e.g. `ووضّح كلامك يا مغيث`), `cleaned_text` (stripped, `ووضح كلامك يا مغيث`), `speaker_age` (string label, e.g. `Elderly -- كبير في السن`), `speaker_gender` (`Male`/`Female`), `speaker_dialect` (**capitalized**: `Najdi`/`Hijazi`, plus `Unknown`). Multi-speaker rows carry the literal sentinel `More than 1 speaker اكثر من متحدث` in **all** demographic columns — there is no separate `speaker_N` column.
* **Decoupled Modality Multi-Routing (blueprint best practice):** Multi-speaker rows are **RETAINED for Tier A** text distillation (natural conversational turn-taking, Q&A, discourse markers — high-value dialogue a monologue lacks) and **PURGED from Tier B** TTS exports (overlapping speech causes Monotonic Alignment Search collapse and cross-talk contamination of single-speaker acoustic timbres). Leaking acoustic constraints into the text layer (blanket rejection) is an anti-pattern that needlessly shrinks the distillation corpus.
...
* **Domain & Persona Focus:** Leverages SADA22 demographic tags (`speaker_age`, `speaker_gender`, `speaker_dialect`) to ensure structural balance across Najdi and Hijazi vectors. Multi-speaker rows are retained strictly for Tier A text distillation (teaching turn-taking) and purged from Tier B TTS acoustic training to avoid cross-talk contamination.
* **Frontier Client Contract (M1.3, verified against api-docs.deepseek.com 2026-08-16):** the distillation client targets `deepseek-v4-flash` via the OpenAI-compatible `https://api.deepseek.com/chat/completions` endpoint. Thinking mode is **enabled by default** on DeepSeek — M1.3 is a schema-restructure task, so the request body explicitly sends `{"thinking": {"type": "disabled"}}` (avoids a 4–6x latency/cost tax); JSON Output mode (`response_format={"type": "json_object"}`) with the word "json" + an example in the prompt; `max_tokens` set to prevent truncation. A single long-lived `httpx.AsyncClient` preserves the TCP/TLS socket pool across 50K-row batches; bounded concurrency (`asyncio.Semaphore`, default 8) maximizes I/O throughput.
* **Resumable Streaming & Zero-Loss Checkpointing (M1.3):** validated dialogues stream to `.jsonl` in append mode with a per-line `flush()` — a crash mid-batch never loses completed samples. A sidecar `<output>.anchors` registry (blake2b of the input row + demographics, deterministic and `PYTHONHASHSEED`-immune) tracks completed anchors; the startup scanner filters the input queue and resumes from sample N+1 without re-calling the API.
* **Failure Semantics & Economics (M1.3):** HTTP 429 honors `Retry-After`; when the header is missing, zero, or non-numeric, the client falls back to exponential backoff + jitter (never a zero-second hot loop). HTTP 402 (payment exhaustion) is **fatal and non-retryable** — the worker pool halts cleanly with an actionable top-up message. Per-sample failures (schema/prose glitches) are logged and skipped (counted in a `DistillationReport`); `--fail-fast` aborts on the first failure for CI/smoke runs.
* **Hardened Lexical Drift Strategy (M1.3):** the drift denylist contains only unambiguous Egyptian/Levantine particles (e.g., `عايز`, `إزاي`, `بدّي`, `هلق`) with word-boundary anchors — native Saudi particles (`عشان`, `مين`, `ليش`) are never rejected, and spoken spelling variants (`كده`/`كدا`) are canonicalized to `كذا` by the M1.1 normalizer (with clitic preservation: `وكدا` → `وكذا`) instead of dropping the whole dialogue. A soft, local-only SADA22 speaker-dispersion validator flags tokens appearing across <5 distinct recordings (distinct `audio` rows as the documented session proxy — the verified schema has no `speaker_id` column); it reports/triggers regeneration but never gates hermetic CI.
* **Inverted MinHash LSH Index (M1.3):** near-duplicate candidate retrieval uses an inverted hash-bucket index (`band_key → [doc_id]`) instead of an O(N²) pairwise scan; exact Jaccard ≥ 0.75 remains the deterministic final arbiter. Band size 1 keeps 100% recall for the 0.75 threshold by construction. Scope guardrail: MinHash applies **only** to synthetic dialogue documents (50–300+ words) — never Tier B rows (acoustic invariance requires repeated phrases) and never raw M1.2 rows; short texts (<6 words) bypass shingling and use exact-match handling only.
